...
void remove_notes(char grid[grid_size][grid_size], int row, int col, char input){ //this is working!
	...
	for (int i=0;i<9;i++){
    	...
    	if (grid[row][i] == '0'){
    		...
    		//cout << "ROW Note removed on: " << row << ' ' << i << "\nRemoved: " << input << endl;
    	}
    	
    	if (grid[i][col] == '0'){
    		
    		notes[i][col].erase(input);
    		//cout << "COL Note removed on: " << i << ' ' << col << "\nRemoved: " << input << endl;
		...
		if (grid[box_start_row + (i % 3)][box_start_col] == '0'){
			...
			//cout << "BOX Note removed on: " << box_start_row + (i % 3) << ' ' << box_start_col << "\nRemoved: " << input << endl <<endl;
...
void build_notes(char grid[grid_size][grid_size]){ //this is working!
	for (int i=0;i<grid_size;i++){
		for (int j=0;j<grid_size;j++){
			if (grid[i][j] == '0'){
